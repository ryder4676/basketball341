...
const updatePlayer = async (req, res) => {
  try {
    ...
    // Handle errors by logging them and responding with a 500 Internal Server Error status
    console.error(
      "Error updating player: Make sure you have all fields required",
      error,
    );
    res
      .status(500)
      .json({
        error: "Error updating player: Make sure you have all fields required",
      });
  }
};
...
const deletePlayer = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ error: "Error deleting player: Use an appropriate player Id" });
  }
};
